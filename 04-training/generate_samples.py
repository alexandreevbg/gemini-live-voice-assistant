...
def generate_samples(
    ...
    model: Union[str, Path] = _DIR / "models" / "bg_BG-dimitar-medium.onnx",
    batch_size: int = 1,
    ...
    **kwargs,
    ...
    # Download model if it is the default one and missing
    if model_path.name == "bg_BG-dimitar-medium.onnx" and not model_path.exists():
        _LOGGER.info("Downloading Bulgarian model...")
        model_path.parent.mkdir(parents=True, exist_ok=True)
        import urllib.request
        urllib.request.urlretrieve("https://huggingface.co/rhasspy/piper-voices/resolve/main/bg/bg_BG/dimitar/medium/bg_BG-dimitar-medium.onnx", model_path)
        urllib.request.urlretrieve("https://huggingface.co/rhasspy/piper-voices/resolve/main/bg/bg_BG/dimitar/medium/bg_BG-dimitar-medium.onnx.json", model_path.with_suffix(".onnx.json"))

    if model_path.suffix == ".onnx":
        sess_options = onnxruntime.SessionOptions()
        torch_model = onnxruntime.InferenceSession(str(model_path), sess_options, providers=["CUDAExecutionProvider", "CPUExecutionProvider"] if torch.cuda.is_available() else ["CPUExecutionProvider"])
        _LOGGER.info("Successfully loaded the ONNX model")
    else:
        torch_model = torch.load(model_path)
        torch_model.eval()
        _LOGGER.info("Successfully loaded the PyTorch model")

    if not isinstance(torch_model, onnxruntime.InferenceSession) and torch.cuda.is_available():
        torch_model.cuda()
    ...
    while speakers_batch:
        ...
        with torch.no_grad():
            ...
            if isinstance(torch_model, onnxruntime.InferenceSession):
            ...
            for i, clip_phoneme_index in enumerate(clip_indexes_by_batch):
                if clip_phoneme_index is not None and phoneme_samples is not None:
                ...
                if phoneme_samples is not None:
...
def generate_audio_onnx(model, phoneme_ids, length_scale, noise_scale, noise_scale_w):
    lengths = np.array([len(i) for i in phoneme_ids], dtype=np.int64)
    max_len = np.max(lengths)
    input_ids = np.zeros((len(phoneme_ids), max_len), dtype=np.int64)
    ...
    if "sid" in input_names:
        inputs["sid"] = np.zeros(len(phoneme_ids), dtype=np.int64)
    ...
    return torch.from_numpy(audio), None
...
def main() -> None:
    ...
    parser.add_argument("--model", default=_DIR / "models" / "bg_BG-dimitar-medium.onnx")
