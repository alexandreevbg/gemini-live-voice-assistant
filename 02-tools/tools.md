...
> **If you plan to also install PipeWire+AEC** (`03-environment.md`), raspotify's packaged system service will stop working once PipeWire takes ownership of the audio hardware (`snd_pcm_open ... Host is down`) — this happens regardless of which you install first. See `03-environment.md`'s "Raspotify vs PipeWire" section for the one-time fix (swapping to a user-level service). If you're not using PipeWire/AEC, no further action is needed here.
