...
```text
limiter_playback:output_MONO   ->   # limiter feeds the AEC reference
  |->aec_output:playback_MONO                    
aec_playback:output_FL              # AEC feeds the speaker
  |-> alsa_output.platform-soc_sound.stereo-fallback:playback_FL
aec_playback:output_FR
...
## 9. Raspotify vs PipeWire (if you installed Raspotify)

If you installed Raspotify (`02-tools.md`'s optional step 3) — whether before or after this section — its packaged **system**-level service tries to open ALSA directly, which conflicts with PipeWire now owning the audio hardware: it crash-loops with `snd_pcm_open ... Host is down (112)`. This happens regardless of install order — it's a standing runtime conflict between two things wanting exclusive ALSA access, not something a particular install sequence avoids.

The fix: mask the system service, then run `librespot` as a **user**-level service instead — it shares your PipeWire session via `pipewire-pulse`, so audio routes through the same limiter/AEC chain set up above.
...
```bash
mkdir -p ~/.config/systemd/user
nano ~/.config/systemd/user/raspotify.service
```
Paste the following, replace `YOUR_USERNAME` with your login (e.g. `chochko`). If `which librespot` returned a different path, fix the `ExecStart` line to match:
```text
[Unit]
...
**Troubleshooting — visible on phone but won't play (`snd_pcm_open ... Host is down`):** the system package is still active. Confirm and fix:
```bash
...
**Next**: to let the voice assistant control playback by voice (play/pause/volume/search), continue to `05-voice_assist/voice_assist.md` §3.1 for Web API authorization.
