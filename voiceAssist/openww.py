...
from .const import ClientData, State # Updated import for State
...
def ww_proc(
    ...
    try:
        ...
        while state.is_running:
            if not ww_state.embeddings_ready.acquire(timeout=1):  # Timeout of 1 second
                continue  # Check state.is_running again
            ...
            while True:
                ...
                for client_id, embeddings_tensor in todo_embeddings.items():
                    ...
                    if probability.item() > THRESHOLD / 2:
                        _LOGGER.debug("probability=%s", probability.item(),)
                    ...
                    with state.clients_lock:
                        ...
                        if probability.item() >= client_data.threshold:
                            ...
                            if client_data.activations >= client_data.trigger_level:
                                ...
                                detection_callback(name=ww_model_key)
