...
GPIO13, active HIGH (TTP223 touch sensor — the IC drives the line itself,
LOW when idle / HIGH when touched, so no internal pull-up is used).
...
class Button:
    ...
    def start(self):
        if not _HAS_GPIO:
            ...
            return
        # pull_up=False → internal pull-down; pressed/touched drives the line HIGH
        # (active HIGH — the TTP223 sensor drives its output directly, it isn't a
        # switch-to-ground, so no internal pull-up is wanted here).
        self._button = _GZButton(BUTTON_PIN, pull_up=False)
