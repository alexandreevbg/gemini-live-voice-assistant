...
class VoiceAssistant:
    def __init__(self):
        ...
        self.wakeword = create_detector(model_path=config.WAKEWORD_MODEL)
        self.button   = Button()
        self.gemini   = None
    ...
    async def _run_dialog(self):
        ...
        # IMPORTANT: only if it's already playing ON Chochko. set_volume()
        # calls _ensure_active(), which transfers playback to Chochko if
        # some OTHER device is currently active — so checking get_current()
        # alone (account-wide "is anything playing") isn't enough: it would
        # hijack playback from e.g. a PC just because the wake word fired,
        # even for a completely unrelated request.
        self._spotify_ducked = False
        if self.spotify.connected:
            try:
                ...
                if current and current.get('playing'):
                    device = await self.loop.run_in_executor(
                        None, self.spotify.get_current_playback_device
                    )
                    if device and 'chochko' in (device.get('name') or '').lower():
                        await self.loop.run_in_executor(
                            None, self.spotify.set_volume, 75
                        )
                        self._spotify_ducked = True
                    else:
                        log.info('Spotify playing on another device — not ducking/transferring')
    ...
    # Below this raw ALSA "Master" percentage, this hardware is inaudible —
    # the control's raw 0-100 range isn't perceptually linear, so the bottom
    # chunk of it does nothing audible. Rescale the user-facing 0-100%
    # onto [MASTER_MIN_PCT, 100] instead of [0, 100], so "quiet" still means
    # something audible rather than silence. Tune by ear: raise if the low
    # end is still too quiet, lower if it's now too loud at the bottom.
    MASTER_MIN_PCT = 33
    ...
    def _handle_volume(self, action: str) -> str:
        def pct_to_raw(pct: int) -> int:
            if pct <= 0:
                return 0
            span = 100 - self.MASTER_MIN_PCT
            return round(self.MASTER_MIN_PCT + span * (pct / 100))

        def set_master(pct: int) -> int:
            pct = max(0, min(100, pct))
            raw = pct_to_raw(pct)
            subprocess.run(
                ['amixer', 'sset', 'Master', f'{raw}%'],
                capture_output=True
            )
            self._volume = pct
            log.info(f'Volume: {pct}% (raw Master {raw}%)')
            return pct
    ...
    async def _end_dialog(self):
        ...
        # Play the chime and switch the LED immediately (after clearing any
        # leftover Gemini audio), same as the start side — don't make the
        # user wait through gemini.disconnect()'s WebSocket close or
        # Spotify's volume-restore call (up to 3s) just to see/hear the
        # "dialog ended" cue. (enter_idle() below still does the rest of
        # the IDLE transition — mic consumer, button handler, self.state —
        # once cleanup actually finishes; only the LED color needs to jump
        # ahead of that.)
        self.speaker.clear()
        chimes.dialog_ended()
        leds.idle()
        ...
        # Restore Spotify volume — only if we actually ducked it. Otherwise
        # this has the exact same hijack risk the duck-check above guards
        # against: set_volume() would transfer playback onto Chochko from
        # whatever device is currently playing, even though we correctly
        # left it alone at duck time.
        if self.spotify.connected and self._spotify_ducked:
            try:
