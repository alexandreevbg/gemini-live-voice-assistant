...
class WakeWordDetector:
    """Base class: shared detect/reset/callback plumbing for MicroWakeWordDetector."""
    ...
    def __init__(self, model_path: str):
        ...
        if not config_path.exists():
            raise FileNotFoundError(
                ...
                f'Point WAKEWORD_MODEL at the .json manifest (and make sure '
        ...
        # probability_cutoff is calibrated per-model in the JSON manifest —
        # that's the only threshold this detector uses.
...
def create_detector(model_path: str) -> WakeWordDetector:
    """Factory - kept as the single construction entry point main.py uses."""
    return MicroWakeWordDetector(model_path)
